...
float rho, last_rho = -10;
float distance_to_the_camera_center = 0.0f, distance_to_the_line = 0.0f;
...
void depth_image_callback(sensor_msgs::Image depth_image) {
  ...
  int line_pixel_length_to_camera_center = (A * depth_image.width / 2 + B * depth_image.height / 2  + C) / sqrt(A*A + B*B);
  if (abs(line_pixel_length_to_camera_center) > depth_image.height / 2) {
    ROS_ERROR_THROTTLE(1, "Line is no longer in vision. line position: %d", line_pixel_length_to_camera_center);
    return;
  }
  distance_to_the_line = *(float *)(pointer + depth_image.step * int(K_matrix(1, 2) - line_pixel_length_to_camera_center) + depth_image.step / 2);
  // ROS_INFO_THROTTLE(1, "Get the distance to the line: %f.", distance_to_the_line);

  // calculate the rho in the virtual vertical image plane
  // first get the pitch angle
  Eigen::Vector3f rpy;
  SO3Utils::quaternion_2_RPY(vehicle_pose, rpy);
  float pitch_radiant = abs(rpy.y());

  rho = (distance_to_the_camera_center * sin(pitch_radiant) + 
          line_pixel_length_to_camera_center / cos(pitch_radiant) / K_matrix(1, 1) * distance_to_the_line) 
        / distance_to_the_line * K_matrix(1, 1);
  rho /= K_matrix(1, 1);
}
